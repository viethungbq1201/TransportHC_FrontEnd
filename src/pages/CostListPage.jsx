...
import { Plus, Search, MoreHorizontal, Pencil, Trash2, X, DollarSign } from 'lucide-react';
...
const CostListPage = () => {
    ...
    const [form, setForm] = useState({ scheduleId: '', costTypeId: '', amount: '', description: '' });
    const [saving, setSaving] = useState(false);
    const [actionMenuId, setActionMenuId] = useState(null);

    const fetchData = async () => { try { const data = await costService.getCostsBySchedule(); setItems(Array.isArray(data) ? data : []); } catch { setItems([]); } finally { setLoading(false); } };
    useEffect(() => { fetchData(); }, []);

    const filtered = items.filter(c => !search || [c.description, c.costType, c.costTypeName].some(v => v?.toLowerCase().includes(search.toLowerCase())));
    ...
    const openCreate = () => { setEditingItem(null); setForm({ scheduleId: '', costTypeId: '', amount: '', description: '' }); setShowModal(true); };
    const handleSubmit = async (e) => { e.preventDefault(); setSaving(true); try { await costService.createCost(form.scheduleId, form); setShowModal(false); fetchData(); } catch (err) { alert(err?.message || 'Error'); } finally { setSaving(false); } };
    ...
    return (
        <div>
            ...
            <div className="flex gap-3 mb-4"><div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search costs..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400" /></div></div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                {loading ? (<div className="flex items-center justify-center h-48"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600" /></div>
                    ...
                    <table className="w-full"><thead><tr className="border-b border-slate-200">
                        ...
                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Schedule</th>
                        ...
                        <tbody>{filtered.map(item => (
                            <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                                <td className="px-5 py-3.5 text-sm font-medium text-slate-900">{item.costTypeName || item.costType || '-'}</td>
                                <td className="px-5 py-3.5 text-sm text-slate-600">{item.scheduleCode || item.scheduleId || '-'}</td>
