...
import { Plus, Search, MoreHorizontal, Pencil, Trash2, X, FolderTree } from 'lucide-react';
...
const CategoryListPage = () => {
    ...
    const openEdit = (item) => { setEditingItem(item); setForm({ name: item.name || '', description: item.description || '' }); setShowModal(true); setActionMenuId(null); };
    ...
    const handleDelete = async (id) => { if (!window.confirm('Delete this category?')) return; try { await categoryService.deleteCategory(id); fetchData(); } catch (err) { alert(err?.message || 'Error'); } setActionMenuId(null); };
