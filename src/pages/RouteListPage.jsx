...
import { Plus, Search, MoreHorizontal, Pencil, Trash2, X, MapPin } from 'lucide-react';
...
const RouteListPage = () => {
    ...
    const [form, setForm] = useState({ name: '', startPoint: '', endPoint: '', distance: '', estimatedTime: '' });
    ...
    const filtered = routes.filter(r => !search || [r.name, r.startPoint, r.endPoint].some(v => v?.toLowerCase().includes(search.toLowerCase())));
    ...
    const openCreate = () => { setEditingItem(null); setForm({ name: '', startPoint: '', endPoint: '', distance: '', estimatedTime: '' }); setShowModal(true); };
    const openEdit = (item) => { setEditingItem(item); setForm({ name: item.name || '', startPoint: item.startPoint || '', endPoint: item.endPoint || '', distance: item.distance || '', estimatedTime: item.estimatedTime || '' }); setShowModal(true); setActionMenuId(null); };

    const handleSubmit = async (e) => { e.preventDefault(); setSaving(true); try { if (editingItem) await routeService.updateRoute(editingItem.id, form); else await routeService.createRoute(form); setShowModal(false); fetchData(); } catch (err) { alert(err?.message || 'Error'); } finally { setSaving(false); } };
    ...
    return (
        <div>
            ...
            <div className="flex gap-3 mb-4">
                <div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by route name or location..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400" /></div>
            </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody>{filtered.map(item => (
                            <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                                ...
                                <td className="px-5 py-3.5 text-sm text-slate-600">{item.startPoint}</td>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center"><div className="absolute inset-0 bg-black/30" onClick={() => setShowModal(false)} />
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
                        <div className="flex items-center justify-between mb-5"><h2 className="text-lg font-semibold text-slate-900">{editingItem ? 'Edit Route' : 'Add New Route'}</h2><button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button></div>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div><label className="block text-sm font-medium text-slate-700 mb-1">Route Name</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20" /></div>
